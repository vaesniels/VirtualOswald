...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if gameStarted == false{
            ...
            let movePipes = SKAction.moveBy(x: -(distance + 200), y: 0, duration: TimeInterval((0.008 * distance)))
        ...
        for touch in touches{
            ...
            if died == true{
                if restartBtn.contains(location){
                    if UserDefaults.standard.string(forKey: "highestScore") != nil {
                    ...
                    if homeBtn.contains(location){
                        self.gameDelegate?.onPlayerDied()
                    }
                }
            } else {
    ...
    func createScene(){
        ...
        birdSprites.append(birdAtlas.textureNamed("oswald"))
