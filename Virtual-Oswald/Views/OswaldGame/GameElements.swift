...
extension GameScene{
    func createBird() -> SKSpriteNode {
        let bird = SKSpriteNode(texture: SKTextureAtlas(named:"player").textureNamed("oswald"))
        bird.size = CGSize(width: 50, height: 65)
    ...
    func createHomeBtn(){
        ...
        homeBtn.position = CGPoint(x: self.frame.width / 2, y: self.frame.height / 2)
        homeBtn.zPosition = 6
        homeBtn.setScale(0)
    ...
    func createScoreLabel() -> SKLabelNode {
        ...
        scoreLbl.fontName = "Poppins Regular"
    ...
    func createHighscoreLabel() -> SKLabelNode {
        ...
        if let highestScore = UserDefaults.standard.object(forKey: "highestScore"){
            highscoreLbl.text = "Topscore: \(highestScore)"
        } else {
            highscoreLbl.text = "Topscore: 0"
        }
        highscoreLbl.zPosition = 5
        highscoreLbl.fontSize = 15
        highscoreLbl.fontName = "Poppins Regular"
        return highscoreLbl
    }
    ...
    func createLogo() {
        ...
        logoImg.size = CGSize(width: 272, height: 120)
    ...
    func createTaptoplayLabel() -> SKLabelNode {
        ...
        taptoplayLbl.text = "tap op het scherm om te staten!"
        taptoplayLbl.fontColor = UIColor(red:0.96, green:0.57, blue:0.12, alpha:1.0)
        taptoplayLbl.zPosition = 5
        taptoplayLbl.fontSize = 20
        taptoplayLbl.fontName = "Poppins Regular"
        return taptoplayLbl
    }
    
    func createWalls() -> SKNode  {
        let flowerNode = SKSpriteNode(imageNamed: "target")
        flowerNode.size = CGSize(width: 40, height: 40)
        flowerNode.position = CGPoint(x: self.frame.width + self.frame.width * 0.1, y: self.frame.height / 2)
        ...
        topWall.position = CGPoint(x: self.frame.width + self.frame.width * 0.1, y: self.frame.height / 2 + self.frame.height / 2.3)
        btmWall.position = CGPoint(x: self.frame.width + self.frame.width * 0.1, y: self.frame.height / 2 - self.frame.height / 2.3)
